...
<style scoped>
.card { cursor: pointer; display: flex; flex-direction: column; background-color: var(--card, #FAFAF8); }
.card:hover .card__img { transform: scale(1.03); }
.card:hover .card__title { color: var(--maroon, #800000); }
.card__media { aspect-ratio: 4 / 5; overflow: hidden; background-color: var(--muted, #F0EDE8); position: relative; }
...
.card__placeholder { width: 100%; height: 100%; background: linear-gradient(135deg,var(--muted, #EDE9E4),#E0DBD5); }
.card__badge { position: absolute; top: 0.75rem; right: 0.75rem; font-size: 0.65rem; letter-spacing: 0.08em; background-color: var(--maroon, #800000); color: #fff; padding: 0.2rem 0.5rem; }
.card__wishlist { position: absolute; top: 0.75rem; left: 0.75rem; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; background-color: rgba(255,255,255,0.9); backdrop-filter: blur(4px); border: none; cursor: pointer; color: var(--muted-foreground, #9e9890); opacity: 0; transform: scale(0.85); transition: opacity 0.2s, transform 0.2s, color 0.2s; }
.card:hover .card__wishlist { opacity: 1; transform: scale(1); }
.card__wishlist--active { opacity: 1 !important; transform: scale(1) !important; color: var(--maroon, #800000); }
.card__wishlist--active svg { fill: var(--maroon, #800000); }
.card__wishlist:hover { color: var(--maroon, #800000); }
.card__footer { padding: 1rem 0.1rem; border-top: 1px solid var(--border, #E8E4DE); }
.card__meta { display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.35rem; }
.card__origin { font-size: 0.7rem; letter-spacing: 0.1em; color: var(--maroon, #800000); text-transform: uppercase; }
.card__roast { font-size: 0.68rem; color: var(--muted-foreground, #9e9890); border: 1px solid var(--border, #E8E4DE); padding: 0.1rem 0.4rem; }
.card__title { font-size: 0.9rem; font-weight: 400; color: var(--foreground, #1a1a1a); margin: 0 0 0.25rem; transition: color 0.2s; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.card__notes { font-size: 0.72rem; color: var(--muted-foreground, #9e9890); margin: 0 0 0.4rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.card__price { font-size: 0.82rem; color: var(--foreground, #3f3a36); margin: 0; }
.card__price span { font-size: 0.7rem; color: var(--muted-foreground, #9e9890); }
...
.card-compact { cursor: pointer; display: flex; flex-direction: column; background-color: var(--card, #FAFAF8); }
.card-compact:hover .card-compact__img { transform: scale(1.05); }
.card-compact__media { aspect-ratio: 1 / 1; overflow: hidden; background-color: var(--muted, #F0EDE8); position: relative; }
...
.card-compact__placeholder { width: 100%; height: 100%; background: linear-gradient(135deg,var(--muted, #EDE9E4),#E0DBD5); }
...
.card-compact__wishlist { position: absolute; top: 0.5rem; left: 0.5rem; width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; background: rgba(255,255,255,0.85); border: none; cursor: pointer; color: var(--muted-foreground, #9e9890); opacity: 0; transition: opacity 0.2s, color 0.2s; }
.card-compact:hover .card-compact__wishlist { opacity: 1; }
.card-compact__wishlist--active { opacity: 1 !important; color: var(--maroon, #800000); }
.card-compact__wishlist--active svg { fill: var(--maroon, #800000); }
.card-compact__body { padding: 0.65rem 0.1rem; }
.card-compact__title { font-size: 0.82rem; font-weight: 400; color: var(--foreground, #1a1a1a); margin: 0 0 0.2rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.card-compact__sub { font-size: 0.68rem; color: var(--muted-foreground, #9e9890); margin: 0; }

.card-h { cursor: pointer; display: flex; flex-direction: row; gap: 0; background-color: var(--card, #FAFAF8); border-bottom: 1px solid var(--border, #E8E4DE); height: 130px; overflow: hidden; }
.card-h:hover { background-color: var(--accent, #F5F2EE); }
.card-h:hover .card-h__title { color: var(--maroon, #800000); }
...
.card-h__placeholder { width: 100%; height: 100%; background: linear-gradient(135deg,var(--muted, #EDE9E4),#E0DBD5); }
...
.card-h__origin { font-size: 0.65rem; letter-spacing: 0.1em; color: var(--maroon, #800000); text-transform: uppercase; }
...
.card-h__wishlist:hover, .card-h__wishlist--active { color: var(--maroon, #800000); }
.card-h__wishlist--active svg { fill: var(--maroon, #800000); }
.card-h__title { font-size: 0.85rem; font-weight: 400; color: var(--foreground, #1a1a1a); margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; transition: color 0.2s; }
.card-h__notes { font-size: 0.7rem; color: var(--muted-foreground, #9e9890); margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.card-h__footer { display: flex; align-items: center; justify-content: space-between; }
.card-h__price { font-size: 0.8rem; color: var(--foreground, #3f3a36); }
.card-h__currency { font-size: 0.68rem; color: var(--muted-foreground, #9e9890); }
.card-h__roast { font-size: 0.66rem; color: var(--muted-foreground, #9e9890); border: 1px solid var(--border, #E8E4DE); padding: 0.1rem 0.4rem; }
</style>
