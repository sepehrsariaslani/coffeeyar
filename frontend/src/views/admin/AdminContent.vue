...
function addTimelineItem() {
  store.content.about.timeline.push({ year: "", title: "", description: "" });
}
function removeTimelineItem(i) {
  store.content.about.timeline.splice(i, 1);
}

// Stats helpers
function addStat() {
  store.content.about.stats.push({ value: "", label: "" });
}
function removeStat(i) {
  store.content.about.stats.splice(i, 1);
}

// Values helpers
function addValue() {
  store.content.about.values.push({ title: "", description: "" });
}
function removeValue(i) {
  store.content.about.values.splice(i, 1);
}

// Story paragraph helpers
function addParagraph() {
  store.content.about.storyParagraphs.push("");
}
function removeParagraph(i) {
  store.content.about.storyParagraphs.splice(i, 1);
}
...
<template>
  <div class="p-10" dir="rtl">
    <div class="mb-8 flex flex-wrap items-end justify-between gap-4">
      <div>
        <h1 class="text-3xl font-light">مدیریت محتوا <span class="text-maroon">.</span></h1>
        <p class="mt-2 text-sm text-muted-foreground">ویرایش متن و محتوای تمام صفحات سایت</p>
      </div>
    ...
    <div class="flex gap-0 border border-border">
      <!-- Sidebar -->
      <nav class="w-48 shrink-0 border-l border-border bg-muted/20">
        <button
          v-for="s in sections"
          :key="s.id"
          type="button"
            ...
            <fieldset class="space-y-4 border border-border p-5">
              ...
              <div v-for="(item, i) in c.about.timeline" :key="i" class="relative border border-border p-4 space-y-3">
                <button
                  ...
                  class="absolute left-3 top-3 p-1 text-muted-foreground hover:text-maroon"
                >
