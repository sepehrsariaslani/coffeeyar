...
<template>
  <!-- ── Variant 1: Classic (minimal, maroon top line) ──────────────────── -->
  ...
  <!-- ── Variant 2: Two-Row (centered logo, nav second row) ────────────── -->
  <header v-else-if="v === 2" class="sticky top-0 z-50 border-b border-border bg-background/90 backdrop-blur-md">
    <div class="mx-auto flex h-14 max-w-7xl items-center justify-between px-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <template v-if="authStore.isLoggedIn">
          <RouterLink to="/account" class="text-muted-foreground hover:text-maroon"><User class="h-5 w-5" /></RouterLink>
        </template>
  ...
  <!-- ── Variant 3: Pill Nav (rounded pills, modern feel) ──────────────── -->
  <header v-else-if="v === 3" class="sticky top-0 z-50 bg-background/80 backdrop-blur-lg">
    <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <template v-if="authStore.isLoggedIn">
          <RouterLink to="/account" class="flex h-9 w-9 items-center justify-center rounded-full bg-muted text-muted-foreground hover:bg-maroon/10 hover:text-maroon transition-colors"><User class="h-4 w-4" /></RouterLink>
        </template>
        <RouterLink v-else to="/auth" class="hidden text-xs text-muted-foreground hover:text-maroon md:block">ورود</RouterLink>
        <button class="md:hidden" @click="open = !open"><X v-if="open" class="h-5 w-5" /><Menu v-else class="h-5 w-5" /></button>
      </div>
  ...
  <!-- ── Variant 4: Bold (thick maroon top bar, large logo) ────────────── -->
    ...
    <div class="bg-maroon px-6 py-2.5">
      <div class="mx-auto flex max-w-7xl items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          <ThemeSwitcher class="[&>button]:text-white/70 [&>button]:hover:text-white" />
  ...
  <!-- Search Modal (shared across all variants) -->
